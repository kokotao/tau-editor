...
use text_editor_lib::{
    ...
    git_status_for_registry, git_unstage_for_registry, resolve_workspace_for_registry,
    WorkspaceRegistry,
};
...
fn reports_modified_files_for_a_resolved_workspace_repository() {
    ...
    let status =
        git_status_for_registry(&registry, &workspace.workspace_id).expect("git status succeeds");
...
fn stages_a_selected_file_and_returns_its_worktree_diff() {
    ...
    let status =
        git_status_for_registry(&registry, &workspace.workspace_id).expect("status succeeds");
    let entry = status
        .entries
        .iter()
    ...
    git_unstage_for_registry(
        &registry,
        &workspace.workspace_id,
        &["notes.md".to_string()],
    )
    .expect("unstage succeeds");
    git_discard_for_registry(
        &registry,
        &workspace.workspace_id,
        &["notes.md".to_string()],
    )
